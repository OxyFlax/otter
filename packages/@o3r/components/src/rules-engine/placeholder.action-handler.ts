...
import { combineLatest, distinctUntilChanged, firstValueFrom, from, map, of, Subject, Subscription, switchMap, withLatestFrom } from 'rxjs';
import {
...
export class PlaceholderRulesEngineActionHandler implements OnDestroy, RulesEngineActionHandler<ActionUpdatePlaceholderBlock> {
  ...
  constructor(store: Store<PlaceholderTemplateStore>, private injector: Injector, private logger: LoggerService) {

    const lang$ = from(import('@o3r/localization')
      // eslint-disable-next-line @typescript-eslint/naming-convention
      .then(({ LocalizationService }) => this.injector.get(LocalizationService))
      .catch(() => null)
    ).pipe(
      switchMap((translateService) => translateService ? translateService.getTranslateService().onLangChange.pipe(
        map(({ lang }) => lang),
  ...
  protected async retrieveTemplate(url: string): Promise<PlaceholderRequestReply> {
    const resolvedUrl$ = await (import('@o3r/dynamic-content')
      // eslint-disable-next-line @typescript-eslint/naming-convention
      .then(({ DynamicContentService }) => this.injector.get(DynamicContentService, null)?.getContentPathStream(url) || of(url))
      .catch(() => of(url)));
