...
describe('new otter application', () => {
  ...
  describe('monorepo', () => {
    ...
    test('should build empty app', () => {
      ...
      // Expect only one file modified inside 'dont-modify-me' project
      expect(diff.all.filter((file) => /projects[\\/]dont-modify-me/.test(file)).length).toBe(1);
      expect(diff.modified).toContain('projects/dont-modify-me/package.json');

      // Expect only one file created outside 'test-app' project
      expect(diff.added.filter((file) => !/projects[\\/]test-app/.test(file)).length).toBe(1);
      expect(diff.added).toContain('.renovaterc.json');
