...
import { findWorkspacePackageJsons, getBestRanges, isRangeBetter } from './version-harmonize';
...
export default createRule<[Options, ...any], 'versionUpdate' | 'error', any>({
  ...
  create: (context, [options]: [Options]) => {
    ...
    const ignoredDependencies = options.ignoredDependencies.map((dep) => new RegExp(dep.replace(/\*/g, '.*')));
              ...
              deps
                ...
                .forEach((depGroup) => {
                  depGroup.properties.forEach((dep) => {
                    ...
                    const bestRange = isRangeBetter(range, bestRanges[name]?.range);
