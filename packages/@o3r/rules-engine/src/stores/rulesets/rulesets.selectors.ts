...
import { computeComponentIdentifier } from '@o3r/core';
  ...
  (ruleSets) =>
    ruleSets
      .reduce((acc: Record<string, string[]>, ruleSet: Ruleset) => {
        ...
        const configName = computeComponentIdentifier(ruleSet.linkedComponent.name, ruleSet.linkedComponent.library);
        acc[configName] ||= [];
